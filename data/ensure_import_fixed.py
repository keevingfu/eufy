...
try:
    ...
    for _, row in df_tk.iterrows():
        # 从"Comments,collections"列中提取评论数
        comments = None
        if 'Comments,collections' in row and pd.notna(row['Comments,collections']):
            comments_str = str(row['Comments,collections'])
            # 尝试提取数字部分
            comments = clean_numeric(comments_str)
